...
export default async function LocaleHome({ params }: LocalePageProps) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-8 px-4 py-6 sm:gap-12 sm:py-10 sm:px-6 md:gap-14 lg:px-0">
      <section className="relative overflow-hidden rounded-2xl border border-slate-800 bg-slate-950 px-5 py-8 shadow-2xl sm:rounded-3xl sm:px-10 sm:py-10">
        ...
        <div className="relative space-y-5 text-slate-50 sm:space-y-7">
          ...
          <div className="space-y-3 text-balance sm:space-y-4">
            <p className="text-xs font-semibold text-amber-300 sm:text-sm">{homeT("learn")}</p>
            <h1 className="text-2xl font-bold leading-[1.1] sm:text-3xl md:text-4xl">{homeT("title")}</h1>
            <p className="max-w-2xl text-sm text-slate-200 sm:text-base md:text-lg">{homeT("subtitle")}</p>
          </div>
          ...
          <ul className="grid grid-cols-1 gap-2.5 sm:gap-3 sm:grid-cols-3" aria-label="Key highlights">
            {heroHighlights.map((highlight) => (
              <li
                key={highlight}
                className="flex items-start gap-2.5 rounded-xl border border-slate-800 bg-slate-900/80 p-3.5 text-xs text-slate-200 shadow-lg sm:gap-3 sm:rounded-2xl sm:p-4 sm:text-sm"
              >
