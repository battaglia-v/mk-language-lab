...
export function QuestsSection() {
  ...
  return (
    <section className="glass-card rounded-3xl p-6 md:p-8 text-foreground" data-testid="profile-quests">
      ...
      <div className="mt-6 space-y-4">
        {isLoading ? (
          Array.from({ length: 2 }).map((_, index) => (
            <div key={`quest-skeleton-${index}`} className="rounded-2xl border border-border/30 bg-muted/20 p-4">
              <div className="h-4 w-1/3 rounded-full bg-muted/30" />
              <div className="mt-2 h-3 w-2/3 rounded-full bg-muted/20" />
          ...
          <div className="rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/10 via-primary/5 to-transparent p-8 text-center">
            <div className="mx-auto max-w-md space-y-3">
              <h3 className="text-xl font-semibold">Daily & Weekly Quests</h3>
          ...
          quests.map((quest: Quest) => (
            <article
              key={quest.id}
              className="rounded-2xl border border-border/30 bg-muted/20 p-4 transition hover:border-primary/40"
            >
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div className="space-y-1">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="text-lg font-semibold">{quest.title}</h3>
                    <Badge variant="outline" className="border-border/40 bg-muted/30 text-xs uppercase tracking-wide">
                      {quest.type}
                    </Badge>
                  </div>
              ...
              <div className="mt-4 space-y-2 text-sm text-muted-foreground">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold text-foreground">{quest.progressPercent}%</span>
                </div>
