...
export function ProfileDashboard({ className, dataTestId }: ProfileDashboardProps) {
    ...
    return (
      <div className={containerClasses} data-testid={dataTestId}>
        <div className="glass-card rounded-3xl border border-border/60 p-6 md:p-8">
          ...
          <h2 className="mt-2 text-2xl font-bold text-foreground md:text-3xl">{t('signIn.subtitle')}</h2>
          <p className="mt-3 max-w-2xl text-sm text-muted-foreground">{t('signIn.body')}</p>
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <Button asChild variant="ghost" size="lg" className="px-6 text-muted-foreground">
  ...
  if (error || !profile) {
    return (
      <div className={containerClasses} data-testid={dataTestId}>
        <div className="glass-card rounded-3xl border border-destructive/40 p-6 text-sm">
          <h2 className="text-lg font-semibold text-foreground">{t('error.title')}</h2>
          <p className="mt-1 text-muted-foreground">{error?.message || t('error.generic')}</p>
