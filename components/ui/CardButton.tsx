...
export function CardButton({
  ...
  const baseStyles = cn(
    ...
    variant === 'primary' && 'bg-gradient-to-r from-primary to-amber-500 text-black border-transparent shadow-lg',
  ...
  const content = (
    <>
      ...
      <div className="flex-1 min-w-0">
        ...
        {subtitle && <p className={cn('text-sm truncate', variant === 'primary' ? 'text-black/70' : 'text-muted-foreground')}>{subtitle}</p>}
      </div>
      {badge}
    </>
  );
