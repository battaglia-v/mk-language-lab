...
export default function ToolsPageClient({ initialMode, labels }: ToolsPageClientProps) {
  ...
  const handleModeChange = useCallback(
    (mode: string) => {
      const newMode = mode as ToolMode;
  ...
  const modeOptions = useMemo(
    () => [
      { value: 'translate', label: labels.translateTab },
      { value: 'analyze', label: labels.analyzeTab },
    ],
    [labels.translateTab, labels.analyzeTab],
  );
  ...
  // Analyzer direction options
  const analyzerDirectionOptions: ReaderDirectionOption[] = useMemo(
    () => [
      {
        id: 'mk-en',
        sourceLang: 'mk',
  ...
  return (
    <PageContainer size="md" className="flex flex-col gap-4 pb-28 sm:pb-6">
          ...
          >
            {history.length === 0 ? (
              ...
              <BottomSheetList
                ...
                itemTestIdPrefix="tools-history-item"
                renderItem={(item) => (
                ...
                {phrases.map((phrase) => (
                  <div key={phrase.id} className="flex items-start gap-3 rounded-lg border border-border p-3">
                    ...
                    <Button
                      ...
                      data-testid={`tools-saved-delete-${phrase.id}`}
                    >
      ...
      {activeMode === 'analyze' && (
        <ReaderWorkspace
          directionOptions={analyzerDirectionOptions}
